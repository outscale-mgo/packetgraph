...
#include <packetgraph/udp-switch.h>
...
struct pg_udp_filter_state {
	...
	enum pg_side to;
	struct pg_udp_port_sw_info *filters;
	int nb_filters;
};
...
static int udp_port_sw_burst(struct pg_brick *brick, enum pg_side from,
			     ...
			     uint64_t pkts_mask, struct pg_error **errp)
{
	...
	struct pg_udp_port_sw_info *filters = state->filters;
	/*
...
static int udp_port_sw_init(struct pg_brick *brick,
		    ...
		    struct pg_error **errp)
{
	...
	struct pg_udp_port_sw_info *pts = fc->ports;

	/* initialize fast path */
	brick->burst = udp_port_sw_burst;
	s->to = fc->to;
	s->filters = g_new(struct pg_udp_port_sw_info, nb_f);
					...
					struct pg_error **errp)
{
	...
	ret = pg_brick_new("udp_port_switch", config, errp);

	pg_brick_config_free(config);
	return ret;
}

static struct pg_brick_ops udp_port_sw_ops = {
	.name		= "udp_port_switch",
	.state_size	= sizeof(struct pg_udp_filter_state),

	.init		= udp_port_sw_init,

	.unlink		= pg_brick_generic_unlink,
};

pg_brick_register(udp_port_sw, &udp_port_sw_ops);
